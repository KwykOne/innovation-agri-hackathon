...
COMMODITY_MAP['BANANA']['model'] = get_loaded_model('models/convosrcimgsetval93train89.h5')
COMMODITY_MAP['FISH']['model'] = get_loaded_model('models/fishmodel.h5')
...
def postData(request):
    try:
        if request.method=="POST":
            ...
            nparray = np.fromstring(imgBytes,np.uint8)
            img_np = cv2.imdecode(nparray,cv2.IMREAD_COLOR)
            cv2.imwrite('sin.png',img_np)

            #  image preprocessing
            img = curr_model['image_preprocessing'](img_np)

            preds=curr_model['model'].predict(tf.expand_dims(img,axis=0))
            logging.info(f'Prediction for the current image : {preds.argmax(axis=1)[0]}')
    ...
    #         # image_cropped,image,wh = crop_pil(image_cropped) # Cropped image PIL TYPE!    
            
    #         # If image is not cropped using crop_pil function uncomment below to get h,w of original image
