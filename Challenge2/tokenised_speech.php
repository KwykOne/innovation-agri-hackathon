...
<body style='width:100%;height:100%;' onload=btnClick()>
  <div class=container style='width:100%;text-align:right;margin:10px;'>
    <div id=cmd style='display:inline-block;padding:10px;font-size:20px;font-weight:800;background-color:green;' onclick=btnClick()>Start</div>
  </div>
  ...
  <div style1='display:none;' id=tokens data-string="" style='display1:none;'>
        ...
        <div id=container>
                <table cellspacing=10px font-size:10px;>
                        <tr>
                                <td>Name</td>
                                <td><input id=name class=attribs></td>
                        </tr>
                        <tr>
                                <td>Price</td>
                                <td><input id=price class=attribs></td>
                        </tr>
                        <tr>
                                <td>Units</td>
                                <td><input id=unit class=attribs></td>
                        </tr>
                        <tr>
                                <td>Quantity</td>
                                <td><input id=quantity class=attribs></td>
                        </tr>
                        <tr>
                                <td>Description</td>
                                <td><input id=description class=attribs></td>
                        </tr>
                </table>
        </div>
  </div>
  ...
  recognition.nomatch = function(event) {
        ...
        function btnClick()
        {
                if (di('cmd').innerHTML == "Start")
                {
                        recognition.start()
                        di('cmd').innerHTML = "Restart"
                }else if (di('cmd').innerHTML == "Restart")
                {
                        di('cmd').innerHTML = "Stopping"
                        recognition.stop()
                        setTimeout(btnClick,500)
                }else if (di('cmd').innerHTML == "Stopping")
                {
                        di('cmd').innerHTML = "Start"
                        setTimeout(btnClick,50)
                }
        }
